...
bl_info = {
    "name": "STP format",
    "author": "Jaume Bellet (mauge)",
    "version": (1, 0, 0),
    "blender": (2, 78, 0),
    "location": "File > Import-Export > Stp",
    "description": "Import-Export STP files",
    "warning": "",
    "wiki_url": "http://mechanicalblender.org"
...
class ImportSTP(Operator, ImportHelper, IOSTLOrientationHelper):
    """Load STP object data"""
    bl_idname = "import_stp.stp"
    bl_label = "Import STP"
    bl_options = {'UNDO'}
...
class ExportSTL(Operator, ExportHelper, IOSTLOrientationHelper):
    """Save STL object data from current scene"""
    bl_idname = "export_scene.stp"
    bl_label = "Export STP"

    filename_ext = ".stp"
    filter_glob = StringProperty(default="*.stp", options={'HIDDEN'})
...
def menu_import(self, context):
    self.layout.operator(ImportSTP.bl_idname, text="Stp (.stp)")
